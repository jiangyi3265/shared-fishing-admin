  ...
  <div class="app-container">
    <el-form :model="queryParams" :inline="true" v-show="showSearch" label-width="80px">
      <el-form-item label="订单号" prop="mallOrderNo">
        <el-input v-model="queryParams.mallOrderNo" clearable style="width:220px" />
      </el-form-item>
    ...
    <el-card class="redeem-card" shadow="never">
      <div style="display:flex;align-items:center;gap:12px;">
        <span style="font-weight:700;">快速确认领取</span>
        <el-input v-model="redeemInput" placeholder="输入订单号" style="width:300px" @keyup.enter="doRedeem" />
        <el-button type="primary" @click="doRedeem" v-hasPermi="['fishing:mallOrder:redeem']">确认</el-button>
        <span style="color:#999;margin-left:8px;">商品订单支付后可直接使用，无需核销码</span>
      </div>
    </el-card>
    ...
    <el-table v-loading="loading" :data="list" style="margin-top:12px">
      ...
      <el-table-column label="操作" width="220">
        <template #default="s">
          ...
          <el-button link type="success" v-if="s.row.status === 1" @click="quickRedeem(s.row)" v-hasPermi="['fishing:mallOrder:redeem']">确认领取</el-button>
    ...
    <el-dialog title="订单详情" v-model="detailOpen" width="560px">
      <el-descriptions :column="2" border v-if="detail">
        ...
        <el-descriptions-item label="领取时间">{{ detail.redeemedTime || '--' }}</el-descriptions-item>
        <el-descriptions-item label="确认人">{{ detail.redeemedBy || '--' }}</el-descriptions-item>
...
const queryParams = ref({ pageNum: 1, pageSize: 10, mallOrderNo: undefined, status: undefined })
...
function statusLabel(s) { return ['待支付', '可使用', '已领取', '已取消'][s] || s }
...
function quickRedeem(row) {
  proxy.$modal.confirm('确认该订单商品已领取？').then(() => redeemMallOrder(row.mallOrderNo))
    .then(() => { proxy.$modal.msgSuccess('已确认领取'); getList() }).catch(() => {})
}
function doRedeem() {
  const orderNo = (redeemInput.value || '').trim()
  if (!orderNo) { proxy.$modal.msgError('请输入订单号'); return }
  redeemMallOrder(orderNo).then(() => { proxy.$modal.msgSuccess('确认成功'); redeemInput.value = ''; getList() })
}
